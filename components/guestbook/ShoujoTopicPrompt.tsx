...
export function ShoujoTopicPrompt({ question, onShuffle, onWrite }: ShoujoTopicPromptProps) {
    ...
    return (
        ...
        <div className="w-full h-full bg-[#FFFDF9] rounded-2xl p-4 flex items-stretch gap-4 border border-dashed border-[#e6d5c3] shadow-sm">

            {/* 🖼️ Chibi: ใช้ h-full + aspect-square เพื่อให้สูงเท่าเนื้อหาเป๊ะๆ และเป็นจัตุรัสเสมอ */}
            <div className={`h-full aspect-square flex-shrink-0 rounded-xl flex items-center justify-center text-3xl shadow-inner border border-white/50 ${currentMood.bg}`}>
                {currentMood.emoji}
            </div>
            ...
            <div className="flex flex-col justify-between py-1 w-full">
                <div>
                    ...
                    <div className="min-h-[2.5rem] flex items-center mt-1 mb-2">
                        <AnimatePresence mode="wait">
                            <motion.h2
                                ...
                                className="font-noto-serif text-base sm:text-lg leading-snug line-clamp-2 text-[#4A3B32] font-medium"
                            >
                                '{question.text}'
                            </motion.h2>
                        </AnimatePresence>
